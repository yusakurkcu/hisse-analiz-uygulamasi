...
if full_stock_list is None:
    ...
    with tab1:
        ...
        if st.button('🧠 TÜM PİYASAYI DERİNLEMESİNE TARA!', type="primary", key="scan_market_button"):
            opportunities = []
    ...
    # --- SEKME 2: DETAYLI HİSSE ANALİZİ (YENİ "RİSK KALKANI" ÖZELLİĞİ İLE GÜNCELLENDİ) ---
    with tab2:
        ...
        if selected_display_name:
            ...
            if data.empty:
                ...
                with col_fin:
                    ...
                    if pe_ratio: st.metric("F/K Oranı (Değerleme)", f"{pe_ratio:.2f}")
                    if revenue_growth: st.metric("Yıllık Gelir Artışı", f"{revenue_growth:.2%}")
                    if profit_margin: st.metric("Net Kâr Marjı", f"{profit_margin:.2%}")
                    if debt_equity: st.metric("Borç/Özkaynak Oranı", f"{debt_equity/100:.2f}")
                ...
                with col_analyst:
                    ...
                    try:
                        ...
                        if reco_df is not None and not reco_df.empty:
                            st.dataframe(reco_df.tail(5)[['firm', 'toGrade']], use_container_width=True)
                        else: st.info("Analist verisi bulunamadı.")
                    except Exception: st.info("Analist verisi alınamadı.")
                ...
                with col_opts:
                    ...
                    if not available_dates:
                        st.warning("Opsiyon vadesi bulunamadı.", icon="⚠️")
                    else:
                        try:
                            ...
                            if recommended_option is not None:
                                st.success(f"**Teknik Öneri:** Görünüm **{sentiment.capitalize()}** olduğu için, bir **{option_type}** opsiyonu düşünülebilir.", icon="💡")
                                st.markdown(f"**Önerilen Kontrat:** ${recommended_option['strike']} {option_type}")
                            else:
                                st.info("Uygun opsiyon stratejisi bulunamadı.", icon="🤔")
